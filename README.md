...
- name: Check License Header
  uses: step-security/skywalking-eyes/header@v0
  # with:
...
> **Note**: In 0.3.0 and earlier versions, GitHub Actions `step-security/skywalking-eyes`
> only works for header check/fix, since 0.4.0, we have a dedicate GitHub Actions
> `step-security/skywalking-eyes/header` for header check/fix and a GitHub Actions
> `step-security/skywalking-eyes/dependency` for dependency resolve/check.
> Now `step-security/skywalking-eyes` is equivalent to `step-security/skywalking-eyes/header` in
> order not to break existing usages of `step-security/skywalking-eyes`.
...
- name: Check Dependencies' License
  uses: step-security/skywalking-eyes/dependency@v0
  # with:
      # log: debug # optional: set the log level. The default value is `info`.
      # config: .licenserc.yaml # optional: set the config file. The default value is `.licenserc.yaml`.
...
```shell
docker run -it --rm -v $(pwd):/github/workspace step-security/skywalking-eyes header check
docker run -it --rm -v $(pwd):/github/workspace step-security/skywalking-eyes header fix
```
...
```dockerfile
FROM step-security/skywalking-eyes:latest
...
[the Docker hub](https://hub.docker.com/r/step-security/skywalking-eyes) for Docker images.

```shell
docker run -it --rm -v $(pwd):/github/workspace ghcr.io/step-security/skywalking-eyes/license-eye header check
docker run -it --rm -v $(pwd):/github/workspace ghcr.io/step-security/skywalking-eyes/license-eye header fix
```

### Compile from Source

```bash
git clone https://github.com/step-security/skywalking-eyes
cd skywalking-eyes
make build
```
...
```bash
go install github.com/step-security/skywalking-eyes/cmd/license-eye@latest
```
...
[Apache License 2.0](https://github.com/step-security/skywalking-eyes/blob/master/LICENSE)
